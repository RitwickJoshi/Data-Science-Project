...
df['hourly'] = df['Salary Estimate'].apply(lambda x: 1 if 'per hour' \
  in x.lower() else 0)
...
sign_remove_hr = sign_remove.apply(lambda x: x.lower().replace('per hour', "")\
...
df['company_txt'] = df.apply(lambda x: x['Company Name'] \
                              if x['Rating'] < 0 else x['Company Name'][:-3],
                              axis = 1)
...
#job desc parsing
#python
df['python_yn'] = df['Job Description'].apply(lambda x: 1 \
                                              if 'python' in x.lower() else 0)

#R studio
df['R_yn'] = df['Job Description'].apply(lambda x: 1 if 'r studio' in x.lower() \
                                          or 'r-studio' in x.lower() else 0)

#spark
df['spark'] = df['Job Description'].apply(lambda x: 1 if 'spark' in x.lower() \
                                          else 0)
...
#excel
df['excel'] = df['Job Description'].apply(lambda x: 1 if 'excel' in x.lower() \
                                          else 0)
